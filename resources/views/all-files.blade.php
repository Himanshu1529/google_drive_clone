                ...
                @if($allfiles->count())
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      @foreach($allfiles as $file)
                          ...
                          <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                              ...
                              <div class="mt-3 flex gap-3 text-sm">
                                ...
                                <form onsubmit="return confirm('Are you Sure to delete this file')" action="{{ route('file.delete', $file->id) }}" method="POST">
                                  @csrf
                                  @method('post')
                        ...
                        @endphp
                        @if($activePlan)
                        <div class="text-center text-xs text-secondaryColor">
                          <a href="{{ route('files.share', ['token' => $file->share_token]) }}" target="_blank">Share</a>
                        </div>
                        @else
                        <div class="text-center cursor-not-allowed text-xs text-secondaryColor">
                          <a href="{{ url('/') }}#price" class="cursor-not-allowed" target="_blank">Share</a>
                        </div>
                        @endif
                      </div>  
                      @endforeach
